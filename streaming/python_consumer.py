...
def main():
    ...
    for message in consumer:
        ...
        key = f"price:{ticker}"
        r.hset(key, mapping={"ticker": ticker, "price": str(price), "volume": str(volume), "timestamp": timestamp})
        r.expire(key, 300)

        # 2. Publish to Redis for UI
        r.publish(
            "price_updates",
            json.dumps({"ticker": ticker, "price": float(price), "volume": int(volume), "timestamp": timestamp}),
        )
                ...
                if diff > 0.05:
                    ...
                    """,
                        (
                            ...
                            "Price Spike",
                            "critical",
                            f"Price moved {diff:.2%} in one tick",
                            price,
                            0.05,
                            timestamp,
                        ),
